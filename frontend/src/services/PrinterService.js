...
class PrinterService {
  ...
  // Ensure the printer is initialized and connected
  async ensureReady() {
    try {
      ...
      const savedAddress = state.settings.connectedPrinterAddress;
      ...
      console.error('Error ensuring printer readiness:', error);
      this.isInitialized = false; // Reset init state on failure
      this.connectedAddress = null;
      throw new Error('PRINTER_CONNECTION_FAILED');
    }
  }
  ...
  // Removes BLEPrinter specific XML tags for rendering in a React Native Text component
  stripPrintTags(rawText) {
    return rawText.replace(/<\/?([CBR])>/g, '');
  }

  // Pure function to generate the text layout of the receipt
  generateReceiptText(cartItems, subTotal, taxAmount, grandTotal, discountAmount = 0, settings, customerObj = null) {
    ...
    if (customerObj && settings.showCustomerDetailsOnInvoice) {
      receiptText += `Customer: ${customerObj.name}\n`;
      if (customerObj.phone) receiptText += `Phone: ${customerObj.phone}\n`;
      if (customerObj.email) receiptText += `Email: ${customerObj.email}\n`;
      if (customerObj.address) {
        receiptText += `Address:\n`;
        const addressLines = customerObj.address.split('\n');
        addressLines.forEach(line => {
          if (line.trim()) receiptText += `${line.trim()}\n`;
        });
      }
      receiptText += separatorLine;
    }
    ...
    const processItems = settings.listProductsSeparately
      ? cartItems.flatMap(item => Array(item.quantity).fill({ ...item, quantity: 1 }))
      : cartItems;
    ...
    const discountLabel = 'Discount:'.padEnd(labelPad, ' ');
    ...
    if (settings.showDiscountOnInvoice && discountAmount > 0) {
      receiptText += `<R>${discountLabel}-${currency}${discountAmount.toFixed(2)}</R>\n`;
    }
  ...
  async printReceipt(cartItems, subTotal, taxAmount, grandTotal, discountAmount = 0, customerObj = null) {
    try {
      ...
      // 1. Send Monochrome O3 Logo to BLE Thermal Print Buffer
      try {
      ...
      // 2. Compile transaction textual receipt
      const receiptText = this.generateReceiptText(cartItems, subTotal, taxAmount, grandTotal, discountAmount, settings, customerObj);
